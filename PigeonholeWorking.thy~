...
proof-
  ...
  then have A'_card: "card A' = card A - 1" by auto
  have "... \<ge> 1" using assms(2) by auto
  hence "card A' \<ge> 1" by (auto simp: A'_card)
...
theorem pigeonhole_app:
  ...
  shows "\<exists>x y. f x = f y"
proof-
  show ?thesis using assms pigeonhole by auto
qed
end